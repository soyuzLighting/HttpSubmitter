...
### 12.1 性能目标（假设：1KB 消息，单分区，8 核 SSD）

| 模式 | 目标写吞吐 | 备注 |
|------|-----------|------|
| 高吞吐档 | ≥ 100,000 msg/s | 压测脚本实测 |
| 零丢失档 | ≥ 10,000 msg/s | 同步刷盘受限 |
| 投递延迟 | 批量 p99 < 100ms | 生产到 Broker ack |

> 实际数值以 `scripts/` 下压测脚本实测结果为准，写入 `docs/test-report.md`。
...
- [x] 协调组件：自研内嵌 Raft
- [x] 刷盘取向：双模式可配置
- [ ] 项目名/包名：暂定 `Herald` / `io.herald`（可改）
- [ ] 性能目标具体数值（当前为 1KB 消息假设下的目标，压测后校准）
